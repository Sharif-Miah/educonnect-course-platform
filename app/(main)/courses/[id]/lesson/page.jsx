...
const Course = async ({ params: { id }, searchParams: { name, module } }) => {
  ...
  if (!lessonToPlay) {
    return (
      <div className="flex flex-col items-center justify-center p-16 text-center bg-white rounded-3xl border border-slate-100 shadow-sm space-y-4 my-8 max-w-lg mx-auto">
        <div className="w-16 h-16 rounded-full bg-indigo-50 text-[#4A3AFF] flex items-center justify-center">
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-16">
      ...
      <div className="bg-slate-950 rounded-3xl overflow-hidden shadow-2xl border-4 border-white ring-1 ring-slate-200/80 relative">
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-7 shadow-sm border border-slate-100/90 space-y-5">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {lessonToPlay.state === "completed" && (
                <span className="bg-emerald-50 text-emerald-600 text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1 border border-emerald-100">
          ...
          <div className="flex items-center gap-2.5 flex-shrink-0">
            {prevLesson ? (
              <Link
                ...
                className="inline-flex items-center gap-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs sm:text-sm font-bold px-4 py-2.5 rounded-2xl transition"
              >
              ...
              <button
                disabled
                className="inline-flex items-center gap-1.5 bg-slate-100 text-slate-400 text-xs sm:text-sm font-bold px-4 py-2.5 rounded-2xl opacity-50 cursor-not-allowed"
              >
                <ChevronLeft className="w-4 h-4" />
                <span className="hidden sm:inline">Previous</span>
              </button>
            ...
            {nextLesson ? (
              ...
              <span className="inline-flex items-center gap-1 bg-emerald-50 text-emerald-700 text-xs sm:text-sm font-extrabold px-4 py-2.5 rounded-2xl border border-emerald-200">
        ...
        <Tabs defaultValue="notes" className="w-full pt-1">
          <TabsList className="bg-slate-100/80 p-1 rounded-2xl h-auto mb-6 flex-wrap">
            <TabsTrigger 
              value="notes"
              className="py-2.5 px-4 rounded-xl font-bold text-xs sm:text-sm data-[state=active]:bg-white data-[state=active]:text-[#4A3AFF] data-[state=active]:shadow-sm transition-all flex items-center gap-1.5"
            >
          ...
          <TabsContent value="notes" className="focus-visible:outline-none space-y-4">
            <div className="prose prose-slate max-w-none text-xs sm:text-sm text-slate-600 leading-relaxed font-normal bg-slate-50/70 p-5 rounded-2xl border border-slate-100">
          ...
          <TabsContent value="about-course" className="focus-visible:outline-none space-y-3">
            <div className="bg-slate-50/70 p-5 rounded-2xl border border-slate-100 space-y-2">
