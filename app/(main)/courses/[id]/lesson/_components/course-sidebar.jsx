...
export const CourseSidebar = async ({ courseId }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="p-5 sm:p-6 border-b border-slate-100 space-y-4 bg-gradient-to-b from-slate-50/70 to-white">
        ...
        {/* Progress Display */}
        <div className="space-y-2 bg-white rounded-2xl p-3.5 border border-slate-100 shadow-xs">
          <div className="flex items-center justify-between text-xs font-bold">
            <span className="text-slate-500 flex items-center gap-1">
              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" />
              <span>{totalCompletedModules} of {totalModules} Modules Done</span>
            </span>
            <span className={totalProgress === 100 ? "text-emerald-600 font-black" : "text-[#4A3AFF] font-black"}>
              {totalProgress}%
            </span>
          </div>
